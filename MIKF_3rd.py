    ...
    def predict(self):
        ...
        for i in range(len(self.ind)):
            ...
            for j in range(len(ind_band)):
                ...
                x_new_old[ind_bandj, :] = torch.mm(self.F[ind_bandj:ind_bandj+1, ind_band], self.x[ind_band, :])
                P_temp = torch.mm(torch.mm(self.F[ind_bandj:ind_bandj+1, ind_band], self.P[ind_band, :][:, ind_band]), self.F[ind_bandj:ind_bandj+1, ind_band].T)
                P_new_old[ind_bandj:ind_bandj+1, ind_bandj:ind_bandj + 1] = P_temp + self.Q[ind_bandj:ind_bandj+1, ind_bandj:ind_bandj + 1]
    ...
    def update(self, y):
        ...
        if isinstance(self.H, multiply_by_H):
            for i in range(len(self.ind)):
                ...
                for j in range(len(ind_band)):
                    ind_bandj = ind_band[j]
                    sigmayx = torch.mm(self.H.Ht_right(P_new_old[ind_bandj, ind_band]), torch.diag(self.D[ind_band_y, ind_band_y]).T)
                    k = torch.mm(sigmayx, torch.inverse(S))
                    x_new_new[ind_bandj, :] = x_new_old[ind_bandj, :] + torch.mm(k, z)
                    sigmaS = torch.mm(k, torch.mm(S, k.T))
                    P_new_new[ind_bandj, ind_bandj:ind_bandj + 1] = P_new_old[ind_bandj,ind_bandj:ind_bandj + 1] - sigmaS
        else:
            for i in range(len(self.ind)):
                ind_band = self.ind[i].numpy().astype(int)
                ...
                for j in range(len(ind_band)):
                    ind_bandj = ind_band[j]
                    k = torch.mm(torch.mm(torch.mm(P_new_old[ind_bandj:ind_bandj+1, ind_band],
