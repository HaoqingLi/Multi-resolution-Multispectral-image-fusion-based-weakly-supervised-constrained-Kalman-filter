...
class multiply_by_H:
    ...
    def H_left(self, X):
        ...
        for i in range(0, X.shape[1]):
            ...
            Xtemp = np.reshape(X[:,i:i+1], (int(X.shape[0]/self.dims_high[0][2]), self.dims_high[0][2]))
            XtempVec = Xtemp[:, 0]
            for k in range(1, self.dims_high[0][2]):
                XtempVec = np.concatenate((XtempVec, Xtemp[:, k]), axis=0)
            ## X[:, i] = XtempVec
            for j in range(0, self.num_bands):
                ...
                tmp_Xi_band_j = XtempVec[dims_past:(dims_past + nr_j * nc_j)]
            ...
            Bi_temp = Bi[0 : int((nr_j* nc_j)/self.d[0]**2)]
            for j in range(1, self.num_bands):
                Bi_temp = np.concatenate((Bi_temp, Bi[int((nr_j * nc_j)/self.d[0]**2)*j:int((nr_j * nc_j)/self.d[0]**2)*(j+1)]), axis=1)
            Bi = np.reshape(Bi_temp, Bi.shape)
            if i == 0:
