    ...
    def predict(self):
        ...
        for i in range(len(self.ind)):
            ...
            for j in range(int(len(ind_band)/self.nb)):
                ...
                x_new_old[ind_bandj, :] = torch.mm(self.F[ind_bandj, :][:, ind_band], self.x[ind_band, :])
                P_temp = torch.mm(torch.mm(self.F[ind_bandj, :][:, ind_band], self.P[ind_band, :][:, ind_band]),
                                  self.F[ind_bandj, :][:, ind_band].T)
                for k in range(len(ind_bandj)):
                    P_new_old[ind_bandj, ind_bandj[k]:ind_bandj[k] + 1] = P_temp[:, k:k+1] + \
                                                                    self.Q[ind_bandj, ind_bandj[k]:ind_bandj[k] + 1]
        self.x = x_new_old
        self.P = P_new_old
        ...
        if isinstance(self.H, multiply_by_H):
            for i in range(len(self.ind)):
                ...
                for j in range(int(len(ind_band) / self.nb)):
                    ...
                    for k in range(len(ind_bandj)):
                        P_new_new[ind_bandj, ind_bandj[k]:ind_bandj[k] + 1] = P_new_old[ind_bandj,
                                                                   ind_bandj[k]:ind_bandj[k] + 1] - sigmaS[:, k:k+1]
        else:
            for i in range(len(self.ind)):
                ind_band = self.ind[i].numpy().astype(int)
                ...
                for j in range(int(len(ind_band) / self.nb)):
                    ind_bandj = ind_band[j * self.nb:(j + 1) * self.nb]
                    k = torch.mm(torch.mm(torch.mm(P_new_old[ind_bandj, :][:, ind_band],
